...
class TestTwilightHelperBackend(unittest.TestCase):
    ...
    @patch("app.get_latest_log_file")
    @patch("app.log_parser.LogParser")
    def test_get_current_status_endpoint_parser_error(
        self, mock_parser_class: MagicMock, mock_get_log_file: MagicMock
    ) -> None:
    ...
    @patch("app.get_latest_log_file")
    def test_get_current_status_endpoint_configured_file_not_found(
        self, mock_get_log_file: MagicMock
    ) -> None:
    ...
    def test_shutdown_endpoint(self) -> None:
        ...
        data = json.loads(response.data)
        self.assertTrue(
            ("status" in data and data["status"] == "shutting down")
            or ("error" in data and "Werkzeug" in data["error"])
            or ("error" in data)
        )
    ...
    @patch("pathlib.Path")
    def test_get_latest_log_file_function(
        self, mock_path: MagicMock, mock_load_config: MagicMock
    ) -> None:
        """Test get_latest_log_file function"""
        # Mock config
        mock_load_config.return_value = {"log_file_path": None, "log_directory": "/test/directory"}
    ...
    def test_error_handling(self) -> None:
        ...
        response = self.app.put(
            "/api/config", data="invalid json data", content_type="application/json"
        )
        self.assertEqual(response.status_code, 400)

        # Test with missing content type
        response = self.app.put("/api/config", data=json.dumps({"test": "data"}))
        self.assertEqual(response.status_code, 400)
